...
export class BaseRepository<T extends Document> {
  ...
  async getByCondition(
    ...
    return this.model.find(filter, field, option).populate(path, select, model, match);
  }
  ...
  async populate(result: T[], options: PopulateOptions | Array<PopulateOptions> | string) {
  ...
  async findByConditionAndUpdate(filter: FilterQuery<T>, update: UpdateQuery<T>) {
    return this.model.findOneAndUpdate(filter, update);
  }

  async updateMany(filter: FilterQuery<T>, update: UpdateQuery<T>, option?: QueryOptions<T> | null) {
